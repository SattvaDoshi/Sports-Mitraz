...
export default function AdminCategories() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetchApi(endpoint, {
        method,
        isAdmin: true,
        body: fd // FormData doesn't need Content-Type header, it's set automatically
      });
  ...
  const renderCategoryTree = (cats: Category[], level = 0) => {
    return cats.map((cat) => (
      <div key={cat.id} style={{ marginLeft: `${level * 24}px`, marginBottom: "8px" }}>
        <div style={{
          display: "flex",
          ...
          borderLeft: `4px solid ${level === 0 ? "#ed0f63" : level === 1 ? "#a7d900" : "#cbd5e1"}`
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              <div style={{ fontWeight: "bold" }}>
                {cat.name} 
                {!cat.isActive && <span style={{ color: "red", fontSize: "0.8rem", marginLeft: "8px" }}>(Inactive)</span>}
                {cat.isLeaf && <span style={{ color: "#3b82f6", fontSize: "0.8rem", marginLeft: "8px", background: "#eff6ff", padding: "2px 6px", borderRadius: "4px" }}>Product Leaf</span>}
              </div>
              <div style={{ fontSize: "0.8rem", color: "#62686f" }}>/{cat.slug}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={() => openModal(cat)} style={{ padding: "6px 12px", border: "1px solid #e6e9e5", background: "#fff", borderRadius: "4px", cursor: "pointer" }}>Edit</button>
            <button onClick={() => handleDelete(cat.id)} style={{ padding: "6px 12px", border: "1px solid #fecaca", background: "#fef2f2", color: "#ef4444", borderRadius: "4px", cursor: "pointer" }}>Delete</button>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div>
          {categories.length === 0 ? (
            <div style={{ padding: "40px", textAlign: "center", background: "#fff", borderRadius: "8px" }}>No categories found.</div>
          ) : (
      ...
      {isModalOpen && (
        <div style={{
          ...
          <div style={{ background: "#fff", width: "90%", maxWidth: "500px", borderRadius: "12px", padding: "24px", maxHeight: "90vh", overflowY: "auto" }}>
            <h2 style={{ marginTop: 0 }}>{editingCat ? "Edit Category" : "Add Category"}</h2>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div>
                ...
                <select value={formData.parentId} onChange={e => setFormData({...formData, parentId: e.target.value})} style={{ width: "100%", padding: "10px", borderRadius: "6px", border: "1px solid #e6e9e5" }}>
              ...
              <div>
                ...
                {editingCat?.image && !imageFile && (
                  <img src={editingCat.image} alt="Current" style={{ width: "60px", marginTop: "8px", borderRadius: "4px" }} />
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: "20px" }}>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: "8px" }} title="If checked, this category acts as a product itself">
                  <input type="checkbox" checked={formData.isLeaf} onChange={e => setFormData({...formData, isLeaf: e.target.checked})} />
                  Is Leaf (Product)
                </label>
              </div>
